...
public abstract class AbstractStudy {
    ...
    class앞에 abstract를 붙이는 것은 선택사항이나 만약에 추상 메소드르 가지게 된다면
    선택사항이 아닌 필수사항이 된다.

    추상 클래스를 쓰는 이유 : 클래스간의 공통적인 부분을 부모에 작성을 해서 상속받도록 하기 위해
    그리고 추상 메소드를 구현해야하는 강제성!!! 이 존재한다.
    추상 메소드 구현 : implements(구현한다라고 표현한다.) --> 구현부를 만드는 행위
     */

    //추상 메소드 정의(선언부만 있고 구현부가 없는 메소드를 추상 메소드라고 한다. 그리고 abstract 키워드를 붙여야 한다.)
...
class AbstractStudyTest {
    public static void main(String[] args) {
        ...
        as2.sum(); //AbstractStudy는 sum메소드를 알고있다. 따라서 호출이 가능하다.
    }
}
